...
live2d_settings["hitokotoAPI"] = "//v2.jinrishici.com/one.json"; // 一言 API，可选 'lwl12.com', 'hitokoto.cn', 'jinrishici.com'(古诗词)
...
live2d_settings["l2dVerDate"] = "2020.11.12"; // 版本更新日期
live2d_settings["homePageUrl"] = "/dashboard"; // 主页地址，可选 'auto'(自动), '{URL 网址}'
live2d_settings["aboutPageUrl"] = "https://srliforever.ltd/"; // 关于页地址, '{URL 网址}'
...
function loadTipsMessage(result) {
  ...
  window.showWelcomeMessage = function (result) {
    ...
    if (window.location.href == live2d_settings.homePageUrl) {
      ...
      if (document.referrer !== "") {
        ...
        if (window.location.hostname == referrer.hostname) text = referrer_message.localhost[0] + document.title.split(referrer_message.localhost[2])[0] + referrer_message.localhost[1];
        else if (domain == "baidu") text = referrer_message.baidu[0] + referrer.search.split("&wd=")[1].split("&")[0] + referrer_message.baidu[1];
        else if (domain == "so") text = referrer_message.so[0] + referrer.search.split("&q=")[1].split("&")[0] + referrer_message.so[1];
        else if (domain == "google") text = referrer_message.google[0] + document.title.split(referrer_message.google[2])[0] + referrer_message.google[1];
        else {
  ...
  function loadRandTextures() {
    ...
    $.ajax({
      ...
      dataType: "json",
      success: function (result) {
        if (result.textures["id"] == 1 && (modelTexturesId == 1 || modelTexturesId == 0)) showMessage(waifu_tips.load_rand_textures[0], 3000, true);
